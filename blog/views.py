...
def upload(request):
    ...
    if request.method == 'POST':
        ...
        if publish == 'D' and draft == 'P':
            ...
            return redirect('blog:upload')
        post = Post.objects.create(title=title, author=author, postimg=postimg, summary=summary, content=content,
                                   slug=slug, publish_status=publish, draft_status=draft
                                   )
        post.save()
...
def update(request, post_id):
    ...
    return render(request, 'blog/update.html', {'post': updatepost, 'category_list': category_list})
